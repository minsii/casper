...
static inline int dtype_idx_blk_regist_impl(CSP_cwp_dtype_idx_blk_regist_pkt_t * regist_pkt)
{
    ...
    packed_param_sz = regist_pkt->param.count * sizeof(int) + sizeof(MPI_Datatype);
    ...
    /* Create new datatype */
    CSP_CALLMPI(JUMP, PMPI_Type_create_indexed_block(regist_pkt->param.count,
                                                     regist_pkt->param.blocklength,
                                                     arr_disps_ptr, *oldtype_ptr, &newtype));
    CSP_ASSERT(newtype != MPI_DATATYPE_NULL);
    CSPG_DBG_PRINT("DTYPE: registered indexed_block 0x%lx (count %d, blen %d, oldtype 0x%lx)\n",
...
/* *INDENT-OFF* */
CSPG_DEFINE_GENERIC_CWP_ROOT_HANDLER(dtype_idx_blk_regist)
CSPG_DEFINE_GENERIC_CWP_HANDLER(dtype_idx_blk_regist)
/* *INDENT-ON* */
