...
static inline int dtype_vector_regist_impl(CSP_cwp_dtype_vector_regist_pkt_t * regist_pkt)
{
    ...
    /* Create new datatype */
    CSP_CALLMPI(JUMP, PMPI_Type_vector(regist_pkt->param.count, regist_pkt->param.blocklength,
                                       regist_pkt->param.stride, g_oldtype, &newtype));
    CSP_ASSERT(newtype != MPI_DATATYPE_NULL);
    CSPG_DBG_PRINT
        ("DTYPE: registered vector 0x%lx (count %d, blklen %d, stride %d, oldtype 0x%lx)\n",
...
/* *INDENT-OFF* */
CSPG_DEFINE_GENERIC_CWP_ROOT_HANDLER(dtype_vector_regist)
CSPG_DEFINE_GENERIC_CWP_HANDLER(dtype_vector_regist)
/* *INDENT-ON* */
