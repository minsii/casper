...
static inline int dtype_indexed_regist_impl(CSP_cwp_dtype_indexed_regist_pkt_t * regist_pkt)
{
    ...
    packed_param_sz = regist_pkt->param.count * sizeof(int) * 2 + sizeof(MPI_Datatype);
    ...
    /* Create new datatype */
    CSP_CALLMPI(JUMP, PMPI_Type_indexed(regist_pkt->param.count, arr_blens_ptr, arr_disps_ptr,
                                        *oldtype_ptr, &newtype));
    CSP_ASSERT(newtype != MPI_DATATYPE_NULL);
    CSPG_DBG_PRINT("DTYPE: registered indexed 0x%lx (count %d, oldtype 0x%lx)\n",
...
/* *INDENT-OFF* */
CSPG_DEFINE_GENERIC_CWP_ROOT_HANDLER(dtype_indexed_regist)
CSPG_DEFINE_GENERIC_CWP_HANDLER(dtype_indexed_regist)
/* *INDENT-ON* */
