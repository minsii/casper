...
static inline int type_dup_impl(MPI_Datatype oldtype, MPI_Datatype * newtype)
{
    int mpi_errno = MPI_SUCCESS;
    CSP_cwp_pkt_t pkt;
    CSP_cwp_dtype_dup_regist_pkt_t *dup_regist_pkt = &pkt.u.fnc_dtype_dup_regist;
    int i = 0, lrank = 0;
    MPI_Datatype *g_oldtypes = NULL;
    MPI_Request *reqs = NULL;

    /* Locally create new datatype */
    CSP_CALLMPI(JUMP, PMPI_Type_dup(oldtype, newtype));
    ...
    CSP_cwp_init_pkt(CSP_CWP_FNC_DTYPE_DUP_REGIST, &pkt);
    dup_regist_pkt->user_local_root = lrank;

    mpi_errno = CSPU_cwp_issue(&pkt);
    CSP_CHKMPIFAIL_JUMP(mpi_errno);

    CSP_DBG_PRINT("DTYPE: dup 0x%lx (oldtype 0x%lx)\n", (MPI_Aint) newtype, (MPI_Aint) oldtype);
...
int MPI_Type_dup(MPI_Datatype oldtype, MPI_Datatype * newtype)
{
    ...
    mpi_errno = type_dup_impl(oldtype, newtype);
    CSP_CHKMPIFAIL_JUMP(mpi_errno);
