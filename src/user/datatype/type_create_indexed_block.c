...
static inline int type_indexed_block_impl(int count, int blocklength,
                                          const int array_of_displacements[],
                                          MPI_Datatype oldtype, MPI_Datatype * newtype)
{
    int mpi_errno = MPI_SUCCESS;
    CSP_cwp_pkt_t pkt;
    CSP_cwp_dtype_idx_blk_regist_pkt_t *regist_pkt = &pkt.u.fnc_dtype_idx_blk_regist;
    int i = 0, lrank = 0, off = 0;
    char *packed_params = NULL;
    int packed_param_sz = 0;
    int *arr_disps_ptr = NULL;
    MPI_Datatype *g_oldtype_ptr = NULL;

    /* Locally create new datatype */
    CSP_CALLMPI(JUMP, PMPI_Type_create_indexed_block(count, blocklength,
    ...
    CSP_cwp_init_pkt(CSP_CWP_FNC_DTYPE_IDX_BLK_REGIST, &pkt);
    ...
    CSP_DBG_PRINT("DTYPE: create indexed_block 0x%lx (count %d, blen %d, oldtype 0x%lx)\n",
                  (MPI_Aint) newtype, regist_pkt->param.count, regist_pkt->param.blocklength,
                  (MPI_Aint) oldtype);

    /* Prepare other parameters and send to each ghost. */
    packed_param_sz = count * sizeof(int) + sizeof(MPI_Datatype);
                                  ...
                                  MPI_Datatype oldtype, MPI_Datatype * newtype)
{
    ...
    if (CSP_IS_DISABLED || CSP_IS_MODE_DISABLED(PT2PT))
        return PMPI_Type_create_indexed_block(count, blocklength, array_of_displacements,
                                              oldtype, newtype);

    mpi_errno = type_indexed_block_impl(count, blocklength, array_of_displacements,
                                        oldtype, newtype);
    CSP_CHKMPIFAIL_JUMP(mpi_errno);
