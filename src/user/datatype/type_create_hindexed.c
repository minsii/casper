...
static inline int type_hindexed_impl(int count, const int *array_of_blocklengths,
                                     const MPI_Aint * array_of_displacements,
                                     MPI_Datatype oldtype, MPI_Datatype * newtype)
{
    int mpi_errno = MPI_SUCCESS;
    CSP_cwp_pkt_t pkt;
    CSP_cwp_dtype_hindexed_regist_pkt_t *regist_pkt = &pkt.u.fnc_dtype_hindexed_regist;
    ...
    CSP_CALLMPI(JUMP, PMPI_Type_create_hindexed(count, array_of_blocklengths,
    ...
    CSP_cwp_init_pkt(CSP_CWP_FNC_DTYPE_HINDEXED_REGIST, &pkt);
    ...
    CSP_DBG_PRINT("DTYPE: create hindexed 0x%lx (count %d, oldtype 0x%lx)\n",
                  (MPI_Aint) newtype, regist_pkt->param.count, (MPI_Aint) oldtype);

    /* Prepare other parameters and send to each ghost. */
    packed_param_sz = count * sizeof(int) + count * sizeof(MPI_Aint) + sizeof(MPI_Datatype);
                             ...
                             MPI_Datatype oldtype, MPI_Datatype * newtype)
{
    ...
    if (CSP_IS_DISABLED || CSP_IS_MODE_DISABLED(PT2PT))
        return PMPI_Type_create_hindexed(count, array_of_blocklengths, array_of_displacements,
                                         oldtype, newtype);

    mpi_errno = type_hindexed_impl(count, array_of_blocklengths, array_of_displacements,
                                   oldtype, newtype);
    CSP_CHKMPIFAIL_JUMP(mpi_errno);
