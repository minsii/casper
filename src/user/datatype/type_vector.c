...
static inline int type_vector_impl(int count, int blocklength, int stride, MPI_Datatype oldtype,
                                   MPI_Datatype * newtype)
{
    int mpi_errno = MPI_SUCCESS;
    CSP_cwp_pkt_t pkt;
    CSP_cwp_dtype_vector_regist_pkt_t *regist_pkt = &pkt.u.fnc_dtype_vector_regist;
    int i = 0, lrank = 0;
    MPI_Datatype *g_oldtypes = NULL;
    MPI_Request *reqs = NULL;

    /* Locally create new datatype */
    CSP_CALLMPI(JUMP, PMPI_Type_vector(count, blocklength, stride, oldtype, newtype));
    ...
    CSP_cwp_init_pkt(CSP_CWP_FNC_DTYPE_VECTOR_REGIST, &pkt);
    regist_pkt->user_local_root = lrank;
    regist_pkt->param.count = count;
    regist_pkt->param.blocklength = blocklength;
    regist_pkt->param.stride = stride;

    mpi_errno = CSPU_cwp_issue(&pkt);
    CSP_CHKMPIFAIL_JUMP(mpi_errno);

    CSP_DBG_PRINT("DTYPE: create vector 0x%lx (count %d, blklen %d, stride %d, oldtype 0x%lx)\n",
                  (MPI_Aint) newtype, regist_pkt->param.count, regist_pkt->param.blocklength,
                  regist_pkt->param.stride, (MPI_Aint) oldtype);
                    ...
                    MPI_Datatype * newtype)
{
    ...
    mpi_errno = type_vector_impl(count, blocklength, stride, oldtype, newtype);
    CSP_CHKMPIFAIL_JUMP(mpi_errno);
