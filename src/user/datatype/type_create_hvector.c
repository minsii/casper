...
static inline int type_hvector_impl(int count,
                                    int blocklength,
                                    MPI_Aint stride, MPI_Datatype oldtype, MPI_Datatype * newtype)
{
    int mpi_errno = MPI_SUCCESS;
    CSP_cwp_pkt_t pkt;
    CSP_cwp_dtype_hvector_regist_pkt_t *regist_pkt = &pkt.u.fnc_dtype_hvector_regist;
    int i = 0, lrank = 0;
    MPI_Datatype *g_oldtypes = NULL;
    MPI_Request *reqs = NULL;

    /* Locally create new datatype */
    CSP_CALLMPI(JUMP, PMPI_Type_create_hvector(count, blocklength, stride, oldtype, newtype));
    ...
    CSP_cwp_init_pkt(CSP_CWP_FNC_DTYPE_HVECTOR_REGIST, &pkt);
    ...
    CSP_DBG_PRINT("DTYPE: create hvector 0x%lx (count %d, blklen %d, stride %ld, oldtype 0x%lx)\n",
                            ...
                            MPI_Aint stride, MPI_Datatype oldtype, MPI_Datatype * newtype)
{
    ...
    mpi_errno = type_hvector_impl(count, blocklength, stride, oldtype, newtype);
    CSP_CHKMPIFAIL_JUMP(mpi_errno);
